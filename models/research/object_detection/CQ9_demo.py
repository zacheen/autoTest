import unittest
import os
# 把 tensorflow 的 warning output 關閉
...
# 初始化 遊戲開始要執行的 Thread
class KPSZNN_Begin_thread (threading.Thread):   #继承父类threading.Thread
    def __init__(self) :
        threading.Thread.__init__(self)

    def run(self) :                   #把要执行的代码写到run函数里面 线程在创建后会直接运行run函数     
        Tool_Main.cut_pic_data("player_money_bef", Tool_Main.glo_var.player_num, Tool_Main.glo_var.round_count%Tool_Main.glo_var.list_len, cover=False) #擷取玩家一開始的分數

# 初始化 遊戲結束要執行的 Thread
class KPSZNN_End_thread (threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
