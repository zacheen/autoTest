...
# debug logger: write to file so logs survive CMD scrollback
...
# Route CategorizedReplayBuffer [DBG sample] / !!BAD ACTIONS!! to the same file.
# This replaces the old hard-coded print/open(...) path.
...
VISUAL_N_STEP = 1
VISUAL_GRAD_CLIP_NORM = 10.0   # Match TransformerDiscreteAgent; keep 10.0 after token_adapter + encoder became trainable.
...
# VisualBackboneV3: YOLOEncoderBase (frozen feature extractor) + cross-attn decoder
...
class VisualAgentV3(VisualAgentCommonMixin):
    ...
    def __init__(self, screen_region=None, grid_h: int = GRID_H, grid_w: int = GRID_W):
        ...
        # try_load_model reads. Current io_log / cuda_debug.log go to current_archive_dir.
        ...
        # Write module ALL_CAPS constants, reward_config dataclass, and selected
        # controller / optimizer fields into one hyperparameters.txt at session start.
        try:
        ...
        # Register hour rollover callback: swap io_log + debug logger. Console
        # messages are printed by SessionArchiveManager.
    ...
    def train_step(self):
        ...
        # Roll over every wall-clock hour; io_log / cuda_debug.log follow the new
        # archive dir via SessionArchiveManager callbacks.
        ...
        # PER beta annealing: linearly move from START to END over
        # VISUAL_PER_BETA_EP episodes. Early weak correction is stable; later beta=1
        # fully corrects priority-sampling bias.
    ...
    def _save_model(self) -> None:
        ...
        # Also write snapshots to the current hour directory. Multiple saves in
        # one hour overwrite, leaving the latest state for that hour. Archive
        # failure does not block canonical save. Same steps as stage1 _save_model.
        try:
