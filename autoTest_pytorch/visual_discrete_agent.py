...
class VisualDiscreteAgent:
    ...
    def train_step(self):
        ...
        yolo_grad_debug = self._module_grad_debug(self.backbone.feature_extractor)
        token_adapter_grad_debug = self._module_grad_debug(self.backbone.token_adapter)
        memory_position_grad_debug = self._module_grad_debug(self.backbone.memory_position)
        query_position_grad_debug = self._module_grad_debug(self.backbone.query_position)
        encoder_grad_debug = self._module_grad_debug(self.backbone.core.transformer)
        decoder_grad_debug = self._module_grad_debug(self.backbone.core.decoder)
        backbone_grad_debug = self._module_grad_debug(self.backbone.core)
        head_grad_debug = self._module_grad_debug(self.q_network)
        ...
        self.tb_writer.add_scalar("debug/query_position_nan_grad_count", query_position_grad_debug["nan_grad_count"], self.total_it)
        self.tb_writer.add_scalar("debug/encoder_nan_grad_count", encoder_grad_debug["nan_grad_count"], self.total_it)
        self.tb_writer.add_scalar("debug/decoder_nan_grad_count", decoder_grad_debug["nan_grad_count"], self.total_it)
        self.tb_writer.add_scalar("debug/backbone_nan_grad_count", backbone_grad_debug["nan_grad_count"], self.total_it)
