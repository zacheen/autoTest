...
# ============================================================
# Stage 1 Discrete 實驗 — 驗證 discrete vs continuous
# ============================================================

GRID_ROWS = 10
GRID_COLS = 10
NUM_ACTIONS = GRID_ROWS * GRID_COLS  # 100


class DiscreteActorNetwork(nn.Module):
    """Discrete Actor: grid state → probability distribution over 100 cells."""

    def __init__(self, embed_dim=256, num_actions=NUM_ACTIONS):
        super().__init__()
        self.encoder = GridEncoder()
        self.attention = HierarchicalAttentionHead(in_channels=128, embed_dim=embed_dim)
        self.fc = nn.Sequential(
    ...
    def forward(self, state):
        ...
        Args:
            state: (B, 12, 10, 10)
        Returns:
            probs: (B, 100) — action probabilities
            log_probs: (B, 100) — log probabilities
        """
        embedding = self.get_embedding(state)
        logits = self.fc(embedding)  # (B, 100)
...
class DiscreteCriticNetwork(nn.Module):
    """Discrete Critic: grid state → Q-values for all 100 cells.
...
DISCRETE_MODEL_PATH = Path('./models/stage1_discrete')


class DiscreteStage1SACAgent:
    """SAC-Discrete agent for Stage 1 pre-training.

    Action space: 100 discrete cells (10×10 grid).
    Uses categorical distribution instead of Gaussian.
    """
    ...
    def __init__(self):
        ...
        # Networks
        self.actor = DiscreteActorNetwork().to(device)
        self.critic = DiscreteCriticNetwork().to(device)
        self.critic_target = DiscreteCriticNetwork().to(device)
        self.critic_target.load_state_dict(self.critic.state_dict())
    ...
    def select_action(self, state, add_noise=True):
        ...
        Args:
            state: (12, 10, 10) tensor
            add_noise: True=從分布 sample, False=argmax
        Returns:
            action: int [0, 99]
        """
        state_batch = state.unsqueeze(0).to(device)
        ...
        self.actor.eval()
        with torch.no_grad():
            probs, _ = self.actor(state_batch)  # (1, 100)
            if add_noise:
    ...
    def train_step(self):
        ...
        # --- Actor update ---
        probs, log_probs = self.actor(state)  # (B, 100)
        with torch.no_grad():
            q1_all, q2_all = self.critic(state)
            min_q = torch.min(q1_all, q2_all)  # (B, 100)

        # SAC-Discrete actor loss: Σ π(a|s) * (α * log π(a|s) - Q(s,a))
    ...
    def on_episode_end(self):
        ...
        self._save_model()
        if self.episode_count % SAVE_EVERY_N_EPISODES == 0:
            print(f"[Discrete] Periodic save at episode {self.episode_count}")
            self.save_persistent()

    def _save_model(self):
        DISCRETE_MODEL_PATH.mkdir(parents=True, exist_ok=True)
        torch.save(self.actor.state_dict(), DISCRETE_MODEL_PATH / 'actor.pth')
        torch.save(self.critic.state_dict(), DISCRETE_MODEL_PATH / 'critic.pth')
        torch.save(self.critic_target.state_dict(), DISCRETE_MODEL_PATH / 'critic_target.pth')
        torch.save({
            'actor_optimizer': self.actor_optimizer.state_dict(),
            'critic_optimizer': self.critic_optimizer.state_dict(),
            'total_it': self.total_it,
            'episode_count': self.episode_count,
            'recent_valid_rates': list(self.recent_valid_rates),
        }, DISCRETE_MODEL_PATH / 'optimizer_state.pth')

    def save_persistent(self):
        """Stratified save to disk."""
        buf = self.replay_buffer
        total = buf.size()
        if total == 0:
            return
        ...
        torch.save({
            ...
            'recent_valid_rates': list(self.recent_valid_rates),
            'persistent_entries': persistent_entries,
        }, DISCRETE_MODEL_PATH / 'training_state.pth')

        saved_rewards = defaultdict(int)
        for entry in persistent_entries:
            saved_rewards[entry['reward']] += 1
        print(f"[Discrete] Persistent save: {len(persistent_entries)} entries")
        print(f"[Discrete] Reward distribution: {dict(saved_rewards)}")

    def try_load_model(self):
        actor_path = DISCRETE_MODEL_PATH / 'actor.pth'
        if actor_path.exists():
            try:
                self.actor.load_state_dict(torch.load(actor_path, map_location=device))
                print("[Discrete] Loaded Actor")
            except Exception as e:
                print(f"[Discrete] Failed to load Actor: {e}")

        critic_path = DISCRETE_MODEL_PATH / 'critic.pth'
        if critic_path.exists():
            try:
                self.critic.load_state_dict(torch.load(critic_path, map_location=device))
                print("[Discrete] Loaded Critic")
            except Exception as e:
                print(f"[Discrete] Failed to load Critic: {e}")

        critic_target_path = DISCRETE_MODEL_PATH / 'critic_target.pth'
        if critic_target_path.exists():
            try:
                self.critic_target.load_state_dict(torch.load(critic_target_path, map_location=device))
                print("[Discrete] Loaded Critic Target")
            except Exception as e:
                print(f"[Discrete] Failed to load Critic Target: {e}")

        opt_path = DISCRETE_MODEL_PATH / 'optimizer_state.pth'
        if opt_path.exists():
            try:
                state = torch.load(opt_path, map_location=device, weights_only=False)
                self.actor_optimizer.load_state_dict(state['actor_optimizer'])
                ...
                if saved_rates:
                    ...
                    avg_valid_rate = np.mean(self.recent_valid_rates)
                    self.alpha = ALPHA_MAX - (ALPHA_MAX - ALPHA_MIN) * avg_valid_rate
                print(f"[Discrete] Loaded optimizer: total_it={self.total_it}, episode={self.episode_count}, alpha={self.alpha:.4f}")
            except Exception as e:
                print(f"[Discrete] Failed to load optimizer state: {e}")

        training_state_path = DISCRETE_MODEL_PATH / 'training_state.pth'
        if training_state_path.exists():
            try:
                state = torch.load(training_state_path, map_location=device, weights_only=False)
                persistent_entries = state.get('persistent_entries', [])
                ...
                if loaded > 0:
                    print(f"[Discrete] Loaded {loaded} replay buffer entries")
            except Exception as e:
                print(f"[Discrete] Failed to load replay buffer: {e}")
