"""
Stage 1 Discrete 實驗 — 驗證 discrete vs continuous action space。

跟 train_stage1.py 相同的設定，唯一差別是動作空間：
- train_stage1.py: continuous (x, y) → 回歸座標
- 這個檔案: discrete 100 格選 1 格 → 分類

執行方式：
  python train_stage1_discrete.py

監控方式：
  tensorboard --logdir runs/stage1_discrete/
"""
...
from RL_Agent import DiscreteStage1SACAgent
...
# ---------- 路徑 ----------
TENSORBOARD_DIR = Path("./runs/stage1_discrete")
CSV_LOG_PATH = Path("./models/stage1_discrete/training_log.csv")


def action_to_grid(action, rows, cols):
    """將 discrete action [0, 99] 轉換為 grid 座標。

    Args:
        action: int in [0, rows*cols-1]
        rows: grid 列數
        cols: grid 行數
    Returns:
        (row, col) 整數座標
...
def compute_reward(result, out_of_bounds=False):
